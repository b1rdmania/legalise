...
- **2026-05-15 Polish pass — closes flag C + a second 204 bug + an auth-shadow bug:**
  - **Settings 204 bug:** `DELETE /api/settings/keys/{provider}` had the same FastAPI `204 must not have a response body` assertion that was fixed on `documents.py`. Patched with `response_class=Response` and explicit `Response(status_code=204)` return. Surfaced because the orchestrator agent tried to mount `TestClient(app)` and tripped it.
  - **Routing-conflict bug:** `GET /api/modules/submissions/config` was returning 401 in dev because the modules router (with auth-gated catch-all `GET /{plugin}/{skill}`) was mounted **before** the submissions router and shadowed `/submissions/config`. Fixed by swapping mount order in `main.py` — submissions router now mounts first under the shared `/api/modules` prefix. Verified live with `curl` against the dev stack.
  - **Full orchestrator eval landed.** New `TestContractReviewOrchestratorE2E` (~190 LoC) in `test_smoke_evals.py`. Uses `TestClient(app)` with `current_user` + `get_session` overrides; mocks all four agents; happy-path asserts `parsed.clauses ≥ 1`, `redlines ≥ 1`, non-empty `summary.executive_summary`, audit row with `module="contract_review"`. Sad-path `C_paused` returns 409. Brings honest eval count to 5 — closes reviewer open-flag C.
  - **Audit-tab module filter** (`frontend/src/matter/tabs/AuditTab.tsx`): the `module` column is now filterable in the UI with per-module entry counts and a separate bucket for middleware `http.*` rows. Closes the "Phase E polish if time" item from `PHASE_INFRA_DELTA.md` §5.
  - **Stale-doc sweep:** ran `grep -rln` for forbidden launch-copy phrases ("capability-gated", "Word template", stale `Five smoke evals`, `Matter export.*v0.1`) — clean across `README.md` + `docs/`.
  - Tests: 60/60 (was 58; +2 orchestrator E2E). Dev stack live on `http://localhost:3000` (frontend) and `http://localhost:8000` (backend) via `docker compose -f infra/docker-compose.yml up`.
