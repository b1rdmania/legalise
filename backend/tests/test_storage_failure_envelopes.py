...
@pytest.mark.asyncio
async def test_generate_docx_storage_write_failure_invokes_audit_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """`app.core.tools.generate_docx.generate_docx` catches
    StorageWriteError and must invoke `audit_failure`, NOT `audit.log`,
    so the failure-provenance row survives the caller's rollback.

    R3 round-2 review fix — same root bug as the upload/download paths.
    """
    from unittest.mock import AsyncMock

    from app.core import api as api_module
    from app.core.storage import StorageWriteError
    from app.core.tools import generate_docx as gd_module
    from app.core.tools.schemas import GenerateDocxInput
    ...
    monkeypatch.setattr(api_module, "audit_failure", _capturing_audit_failure)
    monkeypatch.setattr(gd_module, "audit_failure", _capturing_audit_failure)
    ...
    monkeypatch.setattr(gd_module, "get_storage_backend", lambda: _WriteFails())
    ...
    with pytest.raises(StorageWriteError):
        await gd_module.handle_generate_docx(
            session=AsyncMock(),
            inputs=inputs,
            actor_id=actor_id,
            matter_id=matter_id,
